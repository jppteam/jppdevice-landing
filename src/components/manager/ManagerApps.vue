...
async function applyPackage(filesByPath, label) {
  ...
  try {
    ...
    log(`validated ${result.manifest.app_id} (${result.files.length} files)`)
  } catch (e) {
    invalid.value = e.message
    log(`${t('manager.apps.error')}: ${e.message}`)
  } finally {
...
async function onInstall() {
  ...
  try {
    ...
    for (const file of files) {
      log(`↑ ${file.name} (${(file.data.byteLength / 1024).toFixed(1)} KB)`)
      await s.uploadFile(file.data, `${remoteDir}/${file.name}`)
    }
    log(t('manager.apps.installDone', { app_id: appId }))
  } catch (e) {
...
.apps-err strong { color: var(--error-deep); }
.apps-err .mono { font-size: 0.85rem; color: var(--ink-2); }
.apps-ok { border: 1px solid var(--success); background: var(--success-soft); border-radius: var(--radius); padding: 1.25rem; display: flex; flex-direction: column; gap: 1rem; }
